...
export function WhatIfPanel({ profile, baseResults }: Props) {
  ...
  const [annualExpenses, setAnnualExpenses] = useState(profile.annualExpenses);
  const [realReturn, setRealReturn] = useState(profile.realReturn);
  const whatIfResults = calcFireResults(profile, { monthlyContribution:monthlyContrib, annualExpenses, realReturn });
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[var(--fg-muted)] mt-0.5">Move the sliders to see how changes affect your FIRE date. These don&apos;t save to your profile.</p>
      </div>
      ...
      <div className="space-y-6">
        <SliderRow label="Monthly Contribution" value={monthlyContrib} min={0} max={Math.max(10000,profile.monthlyContribution*3)} step={100} format={(v)=>formatCurrency(v)+"/mo"} baseline={profile.monthlyContribution} onChange={setMonthlyContrib} />
        <SliderRow label="Annual Expenses" value={annualExpenses} min={Math.max(5000,profile.annualExpenses*0.3)} max={profile.annualExpenses*2} step={1000} format={(v)=>formatCurrency(v)+"/yr"} baseline={profile.annualExpenses} onChange={setAnnualExpenses} />
        <SliderRow label="Expected Real Return" value={realReturn} min={0.02} max={0.15} step={0.005} format={(v)=>formatPercent(v)} baseline={profile.realReturn} onChange={setRealReturn} />
      </div>
      <Card>
        <h3 className="text-sm font-semibold text-[var(--fg)] mb-3">Scenario Comparison</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead><tr className="text-[var(--fg-muted)] border-b border-[var(--border)]"><th className="text-left py-2 pr-4">Metric</th><th className="text-right py-2 pr-4">Your Plan</th><th className="text-right py-2">What-If</th></tr></thead>
            <tbody className="divide-y divide-[var(--border)]">
              ...
              <Row label="Annual Expenses" base={formatCurrency(profile.annualExpenses)} whatif={formatCurrency(annualExpenses)} />
            </tbody>
          </table>
        </div>
      </Card>
...
function SliderRow({ label,value,min,max,step,format,baseline,onChange }: { label:string;value:number;min:number;max:number;step:number;format:(v:number)=>string;baseline:number;onChange:(v:number)=>void }) {
  const delta=value-baseline; const pct=baseline!==0?(delta/baseline)*100:0;
  return (
    <div>
      <div className="flex items-center justify-between mb-1">
        ...
        <div className="flex items-center gap-2">
          ...
          {Math.abs(pct)>0.5&&<Badge variant={pct>0?"green":"red"} className="text-xs">{pct>0?"+":""}{pct.toFixed(0)}%</Badge>}
        </div>
      </div>
