...
const LABELS: Record<keyof ExpenseCategories,string> = { housing:"Housing", food:"Food", transport:"Transport", healthcare:"Healthcare", entertainment:"Entertainment", other:"Other" };
export default function ExpensePieChart({ categories, total }: Props) {
  const data = (Object.entries(categories) as [keyof ExpenseCategories,number][]).filter(([,v])=>v>0).map(([k,v])=>({name:LABELS[k],value:v}));
